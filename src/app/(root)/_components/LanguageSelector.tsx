  ...
  return (
    <div className='relative' ref={dropdownRef}>
      ...
      >
        ...
        <ChevronDownIcon className={`size-4 text-gray-400 transition-all duration-300 group-hover:text-gray-300 ${isOpen ? "rotate-100":""}`}/>
      </motion.button>
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          className='absolute top-full left-0 ml-2 w-64 bg-[#1e1e2e]/95 backdrop-blur-xl rounded-xl border border-[#313244] shadow-2xl py-2 z-50'
          >
            ...
            <div className='max-h-[200px] overflow-y-auto overflow-x-hidden'>
              {Object.values(LANGUAGE_CONFIG).map((lang,index)=> {
                ...
                return (
                  ...
                  >
                    <button className={`relative w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-200 ${language === lang.id ? "bf-blue-500/10 text-blue-400":"text-gray-300"} ${isLocked?"opacity-50":"hover:bg-[#262637]"}`} disabled={isLocked} onClick={()=>handleLanguageSelect(lang.id)}>
