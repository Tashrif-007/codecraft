...
export const createSnippet = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        //const user = await ctx.db.query("users").withIndex("by_user_id").filter((q)=>q.eq("userId",identity.subject)).first();
...
export const getSnippetStarCount = query({
    ...
    handler: async (ctx, args) => {
        const stars = await ctx.db.query("stars").withIndex("by_user_id").filter((q)=>q.eq(q.field("snippetId"), args.snippetId)).collect();
...
export const deleteSnippet = mutation({
    ...
    handler: async (ctx,args) => {
        ...
        if(snippet.userId !==identity.subject) {
            throw new Error("Not authed to delete snippet");
        }
...
export const starSnippet = mutation({
    ...
    handler: async (ctx,args) => {
        const identity = await ctx.auth.getUserIdentity();
        if(!identity) throw new Error("Not authed");
